...
export function CellView<T extends Record<string, any>>({
  ...
  const formatValue = (field: ViewFieldConfig, value: any) => {
    ...
    switch (field.type) {
      ...
      case "json": {
        ...
        return (
          <div className="space-y-2">
            ...
            <pre className="bg-muted/50 p-3 rounded-md text-xs font-mono overflow-x-auto max-h-40 overflow-y-auto border">
              {jsonString}
            </pre>
          </div>
        );
      ...
      default:
        // Handle objects by converting them to JSON string
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent className="w-full bg-card backdrop-blur-sm border-border sm:min-w-lg">
          ...
          <div className="flex items-center justify-between w-full">
            <div className="flex gap-3 w-full">
              <Button
                ...
                className="border-border/50 hover:border-border transition-all duration-20 w-full bg-btn-primary"
              >
                Close
              </Button>
            </div>
