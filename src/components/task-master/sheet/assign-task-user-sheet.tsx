...
import {
    getTaskMasterUser,
    createTaskMasterUser,
    deletetaskMasterUser,
} from "@/services/task-master-user";
...
export function TaskMasterUserSheet({ data }: Props) {
    ...
    const fetchAssignedUsers = useCallback(async () => {
        if (!orgName || !taskCode) return;

        try {
            setFetchingAssigned(true);

            const res = await getTaskMasterUser({
                org_name: orgName,
                task_code: taskCode,
                user_id: [],
            });
    ...
    useEffect(() => {
        if (open && orgName && taskCode) {
            fetchUsers(orgName);
            fetchAssignedUsers();
        }
    }, [open, orgName, taskCode, fetchUsers, fetchAssignedUsers]);
    ...
    const handleAddUsers = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const res = await createTaskMasterUser({
                org_name: orgName,
                task_code: taskCode,
                user_id: userIdsList,
            });
    ...
    const handleRemoveUser = async (userId: string) => {
        try {
            ...
            // ✅ Store response to check for errors
            const res = await deletetaskMasterUser({
                org_name: orgName,
                task_code: taskCode,
                user_id: [userId],
            });
            ...
            <Sheet open={open} onOpenChange={handleOpenChange}>
                <SheetContent className="flex flex-col sm:min-w-lg">
                    <SheetHeader className="border-b">
                        <SheetTitle>Assign Users to Task</SheetTitle>
                        <SheetDescription>
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <ConfirmDeleteDialog
                                                        ...
                                                        description={
                                                            <>
                                                                This will remove <strong>{uid}</strong> from this task.
