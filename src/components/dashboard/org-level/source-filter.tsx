...
interface SourceFilterProps {
    ...
    onApplyFilter: (selectedSources: string[]) => void;
    label?: string;
...
export default function SourceFilter({
    ...
    const handleShowResults = () => {
        onApplyFilter([...tempSelectedSources]);
        setSourceDropdownOpen(false);
    ...
    const handleOpenChange = (open: boolean) => {
        if (open) {
            setTempSelectedSources([...selectedSources]); // Initialize temp with current selections
    ...
    return (
        <Popover open={sourceDropdownOpen} onOpenChange={handleOpenChange}>
            ...
            <PopoverContent align="end" className="w-96 p-0 shadow-lg">
                ...
                <div className="px-4 py-2.5 border-b bg-muted/5">
                    <div className="flex items-center justify-between">
                        <div
                            ...
                            onClick={() => {
                                if (filteredItems.length > 0 && filteredItems.every(item => tempSelectedSources.includes(item))) {
                                    // Deselect all filtered items
                                    setTempSelectedSources(prev => prev.filter(item => !filteredItems.includes(item)));
                                } else {
                                    // Select all filtered items
                                    const newSelected = [...new Set([...tempSelectedSources, ...filteredItems])];
                                    setTempSelectedSources(newSelected);
                                }
                            }}
                        >
                            <Checkbox
                                checked={filteredItems.length > 0 && filteredItems.every(item => tempSelectedSources.includes(item))}
                        ...
                        <span className="text-xs text-muted-foreground font-medium">
                            {tempSelectedSources.length} / {availableSources.length} selected
                        </span>
                    </div>
                </div>
                    ...
                    {filteredItems.length === 0 ? (
                        ...
                        <div className="py-2">
                            {filteredItems.map((itemName) => (
                                ...
                                >
                                    <Checkbox
                                        checked={tempSelectedSources.includes(itemName)}
                ...
                <div className="flex items-center justify-between p-4 border-t bg-background">
                    <button
                        ...
                        disabled={tempSelectedSources.length === 0}
